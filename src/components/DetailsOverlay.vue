...
<template>
  <div
    :class="
      'container-flex noi-details-container ' + device + ' ' + options.theme
    "
  >
    <!-- :class="{hide: !show}" -->
    <div class="row" v-if="isMobile">
      <div class="col noi-details-mobile-title">
        {{ flight.flightNumber }}: {{ flight.departure.airport.name }} -
        {{ flight.arrival.airport.name }}
      </div>
    </div>
    <div class="row">
      <div
        class="col-md-4 order-2 order-md-1 noi-details-description-container"
      >
        <div class="noi-details-description-container-gray" v-if="isMobile">
          <div class="row" style="padding: 0 2rem">
            <div
              class="col noi-details-status"
        ...
        <div class="noi-details-description-container-top">
          ...
          <div class="row">
            <div class="col noi-details-from-to-title" v-if="!isMobile">
              {{ $parent.$t("from") }} {{ flight.departure.airport.name }}
              {{ $parent.$t("to") }} {{ flight.arrival.airport.name }}
            </div>
            <div class="col noi-details-from-to-title" v-if="isMobile">
              {{ $parent.$t("from") }} {{ flight.departure.airport.name }} ({{
          ...
          <div class="row">
            <div class="col container noi-details-flight-row-path">
              <div
                class="noi-details-flight-row-plane-line"
              ...
              <span
                >{{ $parent.$t("timeLeft") }}:
                {{ $parent.getShownTimeLeft(flight) }}
                {{ $parent.$t("min") }}</span
              >
            </div>
          </div>
        ...
        <div class="noi-details-description-container-gray">
          <div class="row" style="padding: 0 3rem" v-if="!isMobile">
            ...
            >
              {{ $parent.$t("status." + flight.flightInfo["text"]) }}
              <span
                >{{ $parent.$t("estimatedArrival") }}:
                {{ $parent.asZoneTime(flight.arrival.time) }}</span
              >
            </div>
          </div>
          ...
          <div class="row noi-details-from-to">
            ...
            <div class="col-4">
              <span v-if="flight.departure.airport.gate"
                >{{ $parent.$t("gate") }}
                {{ flight.departure.airport.gate }}</span
              >
            </div>
          </div>
          <div class="row noi-details-from-to">
            ...
            <div class="col-4">
              <span v-if="flight.arrival.airport.gate"
                >{{ $parent.$t("gate") }}
                {{ flight.arrival.airport.gate }}</span
              >
            </div>
          </div>
        </div>
      ...
      <div class="col-md-8 order-1 order-md-2 noi-details-map-container">
        ...
        >
          ...
          >
            <vl-feature>
              <vl-geom-point
                :coordinates="flight.departure.airport.coordinates"
              ></vl-geom-point>
              <vl-style-box>
            ...
            <vl-feature>
              <vl-geom-point
                :coordinates="getLineCoordinates(flight).plane"
              ></vl-geom-point>
              <vl-style-box>
                <vl-style-icon
                  v-if="options.theme == 'skyalps'"
            ...
            <vl-feature>
              <vl-geom-point
                :coordinates="flight.arrival.airport.coordinates"
              ></vl-geom-point>
              <vl-style-box>
                <vl-style-circle :radius="5">
                  <vl-style-fill :color="fillColor"></vl-style-fill>
            ...
            <vl-feature v-if="getLineCoordinates(flight).start">
              <vl-geom-line-string
                :coordinates="getLineCoordinates(flight).start"
              ></vl-geom-line-string>
              <vl-style-box>
                <vl-style>
                  <vl-style-stroke
            ...
            <vl-feature v-if="getLineCoordinates(flight).end">
              <vl-geom-line-string
                :coordinates="getLineCoordinates(flight).end"
              ></vl-geom-line-string>
              <vl-style-box>
                <vl-style>
                  <vl-style-stroke
            ...
            <vl-view
              ...
              :min-zoom="isMobile ? 4.5 : 5"
            ></vl-view>

            <vl-layer-tile id="osm">
              <vl-source-osm
                attributions='<a target="_blank" href="https://opendatahub.com">OpenDataHub.com</a> | &copy; <a target="_blank" href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors.'
                ,
              ></vl-source-osm>
            </vl-layer-tile>
          </vl-map>
...
export default {
  ...
  methods: {
    getFlightAngle(flight) {
      let dx =
        flight.arrival.airport.coordinates[0] * 1.0 -
        flight.departure.airport.coordinates[0] * 1.0;
      let dy =
        flight.arrival.airport.coordinates[1] * 1.0 -
        flight.departure.airport.coordinates[1] * 1.0;
    ...
    reduceCoordinates(coordinates, which, offset) {
      ...
      coordinates[which] = [
        coordinates[which][0] + offsetX,
        coordinates[which][1] + offsetY,
      ];
      return coordinates;
    },
    getLineCoordinates(flight) {
      ...
      switch (planeLineClass) {
        case "end":
          lineCoordinates.start = this.reduceCoordinates(
            [startCoordinates, endCoordinates],
            1,
            offset * 2
          );
          lineCoordinates.plane = this.reduceCoordinates(
            [startCoordinates, endCoordinates],
            1,
            offset
          )[1];
        ...
        case "middle":
          lineCoordinates.start = this.reduceCoordinates(
            [startCoordinates, centerCoordinates],
            1,
            offset
          );
          lineCoordinates.plane = centerCoordinates;
          lineCoordinates.end = this.reduceCoordinates(
            [centerCoordinates, endCoordinates],
            0,
            -offset
          );
          break;
    ...
    getCenterCoordinates(flight) {
      let x =
        (flight.departure.airport.coordinates[0] +
          flight.arrival.airport.coordinates[0]) /
        2;
      let y =
        (flight.departure.airport.coordinates[1] +
          flight.arrival.airport.coordinates[1]) /
        2;
      return [x, y];
    },
